...
type PaletteColorKey = {
  [K in keyof typeof lightPalette]: (typeof lightPalette)[K] extends {
    main: string;
    contrastText: string;
  }
    ? K
...
export const HeaderComponent: React.FC<HeaderComponentProps> = ({
  ...
  const getColor = (colorKey: PaletteColorKey | "textPrimary" | "textSecondary") => {
    if (colorKey === "textPrimary") return theme.palette.text.primary;
    if (colorKey === "textSecondary") return theme.palette.text.secondary;
  ...
  return (
    <Box sx={{ width: "100%", textAlign: align, mb: spacing, ...sx }}>
      <Typography
        variant={titleVariant}
        component={titleVariant as React.ElementType}
        sx={{ color: getColor(titleColor), fontWeight: 600, mb: subtitle ? 0.5 : 0 }}
      >
        {title}
      </Typography>

      {subtitle && (
        <Typography variant="body1" sx={{ color: getColor(subtitleColor), opacity: 0.8 }}>
          {subtitle}
        </Typography>
      )}
    </Box>
